...
const useCalls = (socketRef, setError, callMinimized) => {
 ...
 const playOutgoingRingtone = useCallback(async () => {
  if (!outgoingRingtoneRef.current || !audioEnabled) return;
  try {
 ...
 const emitTrackUpdate = useCallback((tracks) => {
  ...
  if (!socketRef.current || !cs.contact) return;
 ...
 const createPeer = useCallback((initiator, stream, onSignal) => {
  ...
  return peer;
 }, [setError, endCall, attachRemoteStream, stopRingtone]);
 ...
 }, [socketRef, enableAudio, createPeer, setError, playOutgoingRingtone, stopRingtone]);
