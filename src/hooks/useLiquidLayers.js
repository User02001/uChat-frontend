...
/**
 * Liquid layers animation hook (Performance Optimized)
 */
export default function useLiquidLayers(options = {}) {
  const {
    ...
    speed1 = 320,
    speed2 = 260,
    dx = 6, // Increased from 4 for fewer samples
    base1 = 0.34,
    base2 = 0.67,
    amp1 = 0.030,
    amp2 = 0.034,
    ...
    L3 = 980,
    minGap = 0.10,
    targetFPS = 30, // New: Allow FPS control (30 or 60)
    maxDPR = 1.5, // New: Cap device pixel ratio
  } = options;
  ...
  useEffect(() => {
    ...
    function resize() {
      ...
      // OPTIMIZATION 1 & 4: Cap DPR and adjust based on device
      const baseDPR = window.devicePixelRatio || 1;
      if (isMobile || isLowEnd) {
        dpr = 1; // Force 1x on mobile/low-end devices
      } else {
        dpr = Math.min(maxDPR, baseDPR); // Cap at maxDPR (default 1.5)
      }
    ...
    let rafId;
    let frameCount = 0; // OPTIMIZATION 2: Frame skipping

    function tick(now) {
      frameCount++;
      
      // OPTIMIZATION 2: Skip frames to achieve target FPS
      const frameSkip = targetFPS === 30 ? 2 : 1;
  ...
  }, [enabled, speed1, speed2, dx, base1, base2, amp1, amp2, L1, L2, L3, minGap, targetFPS, maxDPR]);
